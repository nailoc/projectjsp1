	...
	//접속정보를 정의 -> 전역변수성격
	       ...
	       // replyPro.jsp 데이터 vo 저장
			...
			public int getNext() {
				...
				String sql = "select * from reply order by no desc";
				try {
					PreparedStatement pstmt = conn.prepareStatement(sql);
					rs = pstmt.executeQuery();
					if(rs.next()) {
			...
			//댓글 출력
			public List<ReplyVo> getReplyList(String no) throws Exception {
				//정렬변수 sort
				//검색어변수 keyword
				List<ReplyVo> rst = new ArrayList<ReplyVo>();
				getConnect();
				//sql
			
				String sql = String.format("select no, id, contents, regdate from reply where board_no='%s'",no);
				rs = stmt.executeQuery(sql);
				while(rs.next()) {
					ReplyVo rr = new ReplyVo();
					rr.setNo(rs.getInt("no"));
					rr.setId(rs.getString("id"));
					rr.setContents(rs.getString("contents"));
					rr.setRegdate(rs.getString("regdate"));
			...
			// reply_writePro.jsp 데이터 vo 저장
			public int saveReply(ReplyVo brdvo) throws Exception {
				int rst=0;
				getConnect();
				StringBuffer sb = new StringBuffer("");
				sb.append("insert into reply (no, board_no, id, contents, regdate)");
				sb.append("\n values (?, ?, ?, ?, ?) ");
				String sql = sb.toString();
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, getNext());
				pstmt.setString(2, brdvo.getBoard_no());
				pstmt.setString(3, brdvo.getId());
			...
			public ReplyVo getBoardByNo(String no) throws Exception {
				ReplyVo rst = new ReplyVo();
				getConnect();
				...
				while(rs.next()) {
					...
					rst.setBoard_no(rs.getString("board_no"));
			...
			//글삭제메소드
			public int deleteReply(String no) throws Exception {
				int rst = 0;
				
				getConnect();
			 
				String sql = String.format("delete from reply where no='%s'",no);
			...
			public int editBoard(ReplyVo bVo) throws Exception {
				
				getConnect();
				
				StringBuffer sb = new StringBuffer("");
				sb.append("update reply set contents=? where no=?");
				String sql = sb.toString();
				pstmt = conn.prepareStatement(sql);
